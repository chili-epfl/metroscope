...
//#define DEBUG_AdaptiveThreshold
#ifdef DEBUG_AdaptiveThreshold
...
void AdaptiveThreshold::run(){
	const IplImage *const tInputImage= *mInputImage;
	if (Image::matchImageFormats(tInputImage, &mOutputImage, true))
	{
		mInputWidth = tInputImage->width;
		mInputHeight = tInputImage->height;

		Image::matchImageFormats(mInputWidth+1, mInputHeight+1, &mIntegralImage);
	}
